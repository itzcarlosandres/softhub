...
<section class="relative pt-24 pb-16 overflow-hidden bg-white dark:bg-gray-900 transition-colors duration-300">
    ...
    <div class="max-w-7xl mx-auto px-6 relative z-10 text-center">
        <h1 class="text-4xl md:text-5xl font-black text-gray-900 dark:text-white tracking-tight mb-4 transition-colors">
            Categoría: <span class="text-blue-600 dark:text-blue-400"><?= htmlspecialchars($category['name']) ?></span>
        </h1>
        <p class="text-lg text-gray-500 dark:text-gray-400 font-medium">
            Mostrando todos los artículos publicados bajo la categoría <?= htmlspecialchars($category['name']) ?>
        </p>
    </div>
</section>
    ...
    <div class="max-w-7xl mx-auto px-6">
        <div class="flex flex-nowrap overflow-x-auto gap-4 scrollbar-hide pb-2" style="scrollbar-width: none;">
            ...
            <?php foreach ($categories as $cat): ?>
                <a href="<?= url('blog/category/' . htmlspecialchars($cat['slug'])) ?>" class="px-5 py-2 whitespace-nowrap <?= $category['id'] == $cat['id'] ? 'bg-blue-600 text-white shadow-lg shadow-blue-500/20' : 'bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-300 border border-gray-200 dark:border-gray-700 hover:border-blue-500' ?> transition-all rounded-full font-bold text-sm">
    ...
    <div class="max-w-7xl mx-auto px-6">
        ...
        <?php if(empty($posts)): ?>
            <div class="text-center py-12 text-gray-500 dark:text-gray-400">
                <i class="fas fa-folder-open text-4xl mb-4 text-blue-300 dark:text-gray-600"></i>
                <p>No hay artículos en esta categoría aún.</p>
                <a href="<?= url('blog') ?>" class="mt-4 inline-block text-blue-600 dark:text-blue-400 hover:underline">← Volver al blog principal</a>
            </div>
        <?php else: ?>
