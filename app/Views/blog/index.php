...
<section class="relative pt-24 pb-16 overflow-hidden bg-white dark:bg-gray-900 transition-colors duration-300">
    ...
    <div class="max-w-7xl mx-auto px-6 relative z-10">
        <div class="text-center max-w-3xl mx-auto">
            ...
            <!-- Buscador del Blog -->
            <form action="<?= url('blog/search') ?>" method="GET" class="max-w-xl mx-auto relative group">
                <input type="text" name="q" placeholder="¿Qué tema estás buscando? Ej: Windows 11, Antivirus..." 
...
<section class="py-6 border-y border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-900/50 transition-colors">
    <div class="max-w-7xl mx-auto px-6">
        <div class="flex flex-nowrap overflow-x-auto gap-4 scrollbar-hide pb-2" style="scrollbar-width: none;">
            <a href="<?= url('blog') ?>" class="px-5 py-2 whitespace-nowrap bg-blue-600 text-white rounded-full font-bold text-sm shadow-lg shadow-blue-500/20"><i class="fas fa-fire mr-1"></i> Todo</a>
            <?php foreach ($categories as $cat): ?>
                <a href="<?= url('blog/category/' . htmlspecialchars($cat['slug'])) ?>" class="px-5 py-2 whitespace-nowrap bg-white dark:bg-gray-800 hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-600 dark:text-gray-300 border border-gray-200 dark:border-gray-700 hover:border-blue-500 transition-all rounded-full font-medium text-sm">
...
<section class="py-16 bg-white dark:bg-gray-900 transition-colors">
    <div class="max-w-7xl mx-auto px-6">
        ...
        <div class="mb-16">
            ...
            <div onclick="window.location='<?= url('blog/' . $featuredPost['slug']) ?>'" class="group cursor-pointer rounded-3xl overflow-hidden bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 shadow-xl shadow-gray-200/50 dark:shadow-none hover:border-blue-500 dark:hover:border-blue-500 transition-all grid grid-cols-1 lg:grid-cols-2">
                <div class="relative h-64 lg:h-full overflow-hidden">
                    <img src="<?= $featuredPost['image'] ? url($featuredPost['image']) : 'https://images.unsplash.com/photo-1550751827-4bd374c3f58b?ixlib=rb-4.0.3&auto=format&fit=crop&w=1200&q=80' ?>" alt="<?= htmlspecialchars($featuredPost['title']) ?>" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700 ease-out">
                    <div class="absolute top-4 left-4">
                        <span class="bg-blue-600 text-white text-xs font-bold uppercase tracking-widest px-3 py-1.5 rounded-lg shadow-lg"><?= htmlspecialchars($featuredPost['category_name'] ?? 'General') ?></span>
                    </div>
                </div>
                <div class="p-8 lg:p-12 flex flex-col justify-center">
                    <div class="flex items-center gap-4 text-xs font-bold text-gray-500 mb-4 uppercase tracking-widest">
                        <span><i class="far fa-calendar mr-1"></i> <?= date('M d, Y', strtotime($featuredPost['created_at'])) ?></span>
                        <span class="w-1 h-1 rounded-full bg-gray-300 dark:bg-gray-600"></span>
                        <span><i class="far fa-eye mr-1"></i> <?= $featuredPost['views'] ?> views</span>
                    </div>
                    ...
                    <p class="text-gray-500 dark:text-gray-400 text-base leading-relaxed mb-8">
                        <?= htmlspecialchars($featuredPost['extract'] ?? 'Lee nuestro artículo destacado...') ?>
                    </p>
                    <div class="flex items-center justify-between mt-auto pt-6 border-t border-gray-100 dark:border-gray-700">
                        <div class="flex items-center gap-3">
                            <div class="w-10 h-10 rounded-full bg-gradient-to-br from-blue-500 to-purple-500 flex items-center justify-center text-white font-bold text-sm shadow-md">
                                <?= strtoupper(substr($featuredPost['author_name'] ?? 'AD', 0, 2)) ?>
                            </div>
                            <div>
                                <h4 class="text-sm font-bold text-gray-900 dark:text-white"><?= htmlspecialchars($featuredPost['author_name'] ?? 'Admin') ?></h4>
                                <p class="text-xs text-gray-500">Editor</p>
                            </div>
                        </div>
                        <a href="<?= url('blog/' . $featuredPost['slug']) ?>" class="text-blue-600 hover:text-blue-700 font-bold text-sm flex items-center gap-2 group-hover:translate-x-1 transition-transform">
